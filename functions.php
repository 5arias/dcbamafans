...
function dcbamafans_setup() {
	 ...
	 * If you're building a theme based on DC Bama Fans, use a find and replace
	 * to change 'dcbamafans' to the name of your theme in all the template files.
	 */
 ...
 */
function dcbamafans_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'dcbamafans_content_width', 640 );
}
add_action( 'after_setup_theme', 'dcbamafans_content_width', 0 );
...
function dcbamafans_scripts() {
	...
	wp_enqueue_script( 'dcbamafans-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'dcbamafans-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
